...
def plot_water_only_individual_range_profiles_per_depth():
    print("\n── Figures W1–W4: Water Only individual profiles ──")
    mask = RANGE_AXIS_CM <= RANGE_MAX_CM

    for idx, depth in enumerate(DEPTHS):
        water_dir     = READINGS_DIR / "Water Only Baseline" / f"{depth}cm"
        expected_dist = SENSOR_HEIGHT_CM - depth

        profiles, peak_bins = [], []
        for rec_dir in sorted(water_dir.iterdir()):
        ...
        for i in range(n):
            ...
            ax.axvline(expected_dist, color="green", ls=":", lw=1, alpha=0.7, label="Expected surface")
            ...
            exp_bin_near = max(0, min(N_BINS - 1, int(round(expected_dist / RANGE_RES_CM))))
            exp_y = prof[exp_bin_near]
            ax.text(expected_dist - 0.5, exp_y + 0.5, f"({expected_dist:.1f} cm, {exp_y:.1f} dBFS)",
                    fontsize=7, color="darkgreen", ha="right", va="bottom",
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", alpha=0.8, ec="none"), zorder=6)
        ...
        fig.suptitle(f"Figure W{idx + 1} — Water Only {depth} cm Fill: All {n} Individual Range Profiles",
                     fontsize=11, y=1.02)
        fig.tight_layout()
        out = OUT_BIAS / f"W{idx + 1}_water_only_{depth}cm_profiles.png"
...
if __name__ == "__main__":
    plot_water_only_individual_range_profiles_per_depth()
