...
def plot_aoa_bartlett_spectra_at_water_surface_bin():
    print("\n── Figure AoA3: AoA spectra by fill depth ──")
        ...
        for depth_dir in sorted(mat_dir.iterdir()):
            ...
            for rec_dir in sorted(depth_dir.iterdir()):
                ...
                try:
                    ...
                    lo   = max(0, surf_bin - 1); hi = min(N_BINS, surf_bin + 2)
    ...
    for idx, depth in enumerate(DEPTHS):
        ...
        for mc in ["water_only", "pebbles_soil", "other_debris"]:
            ...
            bw     = beamwidth(mean_P)
            n      = len(recs)
            ax.plot(AoA_ANGLES_DEG, mean_P, color=MATERIAL_COLORS[mc], lw=2.0,
        ...
        ax.set_title(f"Fill depth {depth} cm  |  Surface bin {surf_bin} ({surf_cm:.1f} cm)", fontsize=9)
        ax.legend(fontsize=7.5, loc="upper right")
        ax.grid(True, alpha=0.25)

    fig.suptitle("Figure AoA3 — AoA Bartlett Spectra by Fill Depth\n"
                 "Water Only vs PebblesAndSoil vs OtherDebris at the water-surface bin",
                 fontsize=11, y=0.97)
    out = OUT_AOA / "4_aoa_spectra_by_depth.png"
...
if __name__ == "__main__":
    plot_aoa_bartlett_spectra_at_water_surface_bin()
